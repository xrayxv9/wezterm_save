...
--- Modified set_images function to fix syntax error
function BackDrops:set_images()
   if (wezterm ~= nil) and (wezterm.glob ~= nil) then
      self.images = wezterm.glob(self.images_dir .. GLOB_PATTERN)
   else
      self.images = {}  -- fallback if wezterm.glob is not available
      if wezterm ~= nil then
         wezterm.log_error("wezterm.glob is not available!")
      else
         print("wezterm.glob is not available!")
      end
   end
   return self
end
...
function BackDrops:_create_opts()
   return {
      ...
      },
      {
         ...
         opacity = 0.70,  -- Adjust this value to change the opacity (0.0 to 1.0)
      },
   }
end
   ...
   return {
      {
         ...
         vertical_offset = '-10%',
         horizontal_offset = '-10%',
         opacity = 1,  -- Adjust this value to change the opacity (0.0 to 1.0)
      },
   }
end
